...
# Прогноз_ЕГЭ2025_Ишимов
from collections import deque

with open('files/26_Прогноз_ЕГЭ2025_Ишимов.txt') as f:
    n, m, k = map(int, f.readline().split())
    points = set()
    for _ in range(n):
...
while points:
    ...
    size = len(ship)
    if size >= 6000:
        min_row = min(r for r, c in ship)
        max_col = max(c for r, c in ship)
        big_ships.append((max_col, min_row))

max_col_global = max(mc for mc, mr in big_ships)
min_row_ans = min(mr for mc, mr in big_ships if mc == max_col_global)
print(min_row_ans, max_col_global)  # 4780 5733
