...
f = open('24_21597.txt').readline()
sp = []
c = sorted(re.findall(r'(?:[0]|[1-5][0-5]*)(?:[-*](?:[0]|[1-5][0-5]*))+', f), key=len)
print(len(c[-1]))
"""
...
file =  open('files/24_18284.txt').readline()

for c in findall(r'(?:L[REVUTIN]*O[REOLUTIN]*V[RVOLUTIN]*E)', file):
    print(len(c))  # 2000031"""

# № 21597 (Уровень: Сложный)
"""№ 21597 (Уровень: Сложный)
Текстовый файл состоит из десятичных цифр и знаков 
арифметических операций «–» и «*» (вычитания и умножения).
Определите максимальное количество символов последовательности, 
являющейся корректным арифметическим выражением
с целыми неотрицательными 6-ричными числами, 
в котором порядок действий слева направо совпадает
с порядком действий по правилам математики.
В этом выражении никакие два знака арифметических операций не стоят рядом, 
в записи чисел отсутствуют незначащие (ведущие) нули.
В ответе укажите количество символов."""
...
s = open('files/24_21597.txt').readline()
...
# №19149
"""№ 19149 (Уровень: Гроб)
(В. Лашин) Текстовый файл состоит из символов ')', '(', знака '+' и цифр 1, 2, 3, 4. 
Определите максимальное количество символов в непрерывной последовательности, 
начинающихся с символа '(' и кончающихся символом ')', 
при этом последовательность является математически корректной и её значение кратно 2.
В этом выражении никакие два знака арифметических операций не стоят рядом, 
в начале и в конце математической последовательности не может стоять знак '+'."""
...
"""№ 21319
(Иглин К.) IP-адреса перебрались и в 24 задание ЕГЭ.
Определите сколько правильно записанных IP-адресов (IPv4) можно найти в текстовом файле.
В ответе укажите только число – количество таких адресов.
*Примечание: запись IP-адреса (IPv4)
является запись в виде четырёх десятичных чисел (от 0 до 255),
разделённых точками, например, 192.168.0.1
Типовой пример: в подстроке 981.52.63.012 – 2 адреса (81.52.63.0, 1.52.63.0)
Разбор тут - ТГ"""
# isn't working
"""from re import *

s = open('files/24_21319.txt').readline()
# s = '981.52.63.012'  # тестовая строка
num = r'((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9]))'
reg = rf'(?=({num}\.{num}\.{num}\.{num}))'
matches = finditer(reg, s)
results = [match.group(1) for match in matches]
print(results)
print(len(results))"""

"""s = open("files/24_21319.txt").read()
c = 0
s = s.split(".")
for i in range(len(s) - 3):
    m = s[i:i + 4]
    if "" not in m and len(m[1]) <= 3 and len(m[2]) <= 3:
        t = ".".join(m)
        for l in range(t.find(".")):
            for r in range(t.rfind(".") + 1, len(t)):
                p = t[l:r + 1]
                if len(p) <= 15:
                    if all(p[:2] != f"0{o}" for o in "0123456789"):
                        if all(f".0{u}" not in p for u in "0123456789"):
